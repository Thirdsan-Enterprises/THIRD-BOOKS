  ...
  <AppLayout>
    <div class="space-y-6">
      ...
      <div class="flex justify-between items-center">
        <h1 class="text-2xl font-bold text-gray-900">Create Bill</h1>
        <router-link to="/bills" class="text-gray-600 hover:text-gray-900">
          Cancel
        </router-link>
      </div>
      ...
      <form @submit.prevent="handleSubmit" class="space-y-6">
        <!-- Vendor & Details Card -->
        <div class="card">
          <h2 class="text-lg font-medium text-gray-900 mb-4">Bill Details</h2>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <router-link to="/vendors" class="text-xs text-primary-600 hover:text-primary-900 mt-1 inline-block">
                + Add new vendor
              </router-link>
            </div>
            ...
            <div class="md:col-span-2">
              <label class="label">Vendor Invoice Number</label>
              <input
        ...
        <div class="flex justify-end space-x-3">
          <router-link to="/bills" class="btn btn-secondary">
            Cancel
          </router-link>
          <button
            type="submit"
...
function updateDueDate() {
  ...
  if (terms === 'immediate') {
    form.value.due_date = form.value.bill_date
  } else {
    const days = parseInt(terms)
    const billDate = new Date(form.value.bill_date)
    const dueDate = new Date(billDate.getTime() + days * 24 * 60 * 60 * 1000)
...
async function handleSubmit() {
  ...
  try {
    const billData = {
      ...
      vendor_invoice_number: form.value.vendor_invoice_number || null,
    ...
    // Submit bill if requested
    if (submitAction.value === 'submit') {
      await api.submitBill(billId)
    }
