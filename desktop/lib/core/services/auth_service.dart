...
class AuthService {
  ...
  // Roles: accountant can post transactions; viewer is read-only.
  ...
  // Offline login: first checks the local pre-seeded users store (Marion,
  // Allan), then falls back to the hash cached from the last online login.
  ...
  // left unchanged so an admin can later change passwords without being overwritten.
