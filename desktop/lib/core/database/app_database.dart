...
class AppDatabase extends _$AppDatabase {
  ...
  Future<List<AssetDepreciationData>> getAllAssetDepreciationSchedules() =>
      select(assetDepreciation).get();

  Future<List<DepreciationEntry>> getAllDepreciationEntries() =>
      select(depreciationEntries).get();

  Future<void> upsertAssetFromMap(Map<String, dynamic> m) async {
    await into(assets).insertOnConflictUpdate(AssetsCompanion(
      id: Value(m['id'] as String),
      assetCode: Value(m['asset_code'] as String),
      name: Value(m['name'] as String),
      description: Value(m['description'] as String?),
      category: Value(m['category'] as String),
      purchasePrice: Value((m['purchase_price'] as num? ?? 0).toDouble()),
      currentValue: Value((m['current_value'] as num? ?? 0).toDouble()),
      accumulatedDepreciation: Value((m['accumulated_depreciation'] as num? ?? 0).toDouble()),
      purchaseDate: Value(DateTime.parse(m['purchase_date'] as String)),
      supplier: Value(m['supplier'] as String?),
      location: Value(m['location'] as String?),
      outletId: Value(m['outlet_id'] as String?),
      isActive: Value(m['is_active'] as bool? ?? true),
      notes: Value(m['notes'] as String?),
      createdAt: Value(m['created_at'] != null ? DateTime.parse(m['created_at'] as String) : DateTime.now()),
      updatedAt: Value(m['updated_at'] != null ? DateTime.parse(m['updated_at'] as String) : DateTime.now()),
    ));
  }

  Future<void> upsertAssetDepreciationFromMap(Map<String, dynamic> m) async {
    await into(assetDepreciation).insertOnConflictUpdate(AssetDepreciationCompanion(
      id: Value(m['id'] as String),
      assetId: Value(m['asset_id'] as String),
      method: Value(m['method'] as String? ?? 'declining_balance'),
      rate: Value((m['rate'] as num).toDouble()),
      period: Value(m['period'] as String? ?? 'yearly'),
      startDate: Value(DateTime.parse(m['start_date'] as String)),
      endDate: Value(m['end_date'] != null ? DateTime.parse(m['end_date'] as String) : null),
      isActive: Value(m['is_active'] as bool? ?? true),
      notes: Value(m['notes'] as String?),
      createdAt: Value(m['created_at'] != null ? DateTime.parse(m['created_at'] as String) : DateTime.now()),
      updatedAt: Value(m['updated_at'] != null ? DateTime.parse(m['updated_at'] as String) : DateTime.now()),
  ...
  Future<void> upsertDepreciationEntryFromMap(Map<String, dynamic> m) async {
    await into(depreciationEntries).insertOnConflictUpdate(DepreciationEntriesCompanion(
      id: Value(m['id'] as String),
      assetId: Value(m['asset_id'] as String),
      assetDepreciationId: Value(m['asset_depreciation_id'] as String),
      journalEntryId: Value(m['journal_entry_id'] as String?),
      date: Value(DateTime.parse(m['date'] as String)),
      depreciationAmount: Value((m['depreciation_amount'] as num? ?? 0).toDouble()),
      bookValueBefore: Value((m['book_value_before'] as num? ?? 0).toDouble()),
      bookValueAfter: Value((m['book_value_after'] as num? ?? 0).toDouble()),
